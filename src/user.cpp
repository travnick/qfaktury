...
void User::init()
{
    ...
    {
        accountsCombo->addItem(settings.value("account" + QString::number(i + 1)).toString());
    }

    for (int i = 0; i < settings.value("telsCount").toInt(); ++i)
    {
        telsCombo->addItem(settings.value("phone" + QString::number(i + 1)).toString());
    }

    for (int i = 0; i < settings.value("faxesCount").toInt(); ++i)
    {
        faxesCombo->addItem(settings.value("fax" + QString::number(i + 1)).toString());
    }

    for (int i = 0; i < settings.value("emailsCount").toInt(); ++i)
    {
        emailsCombo->addItem(settings.value("email" + QString::number(i + 1)).toString());
    }
        ...
        || (sett().value("validation").toBool() && checkAll()))
    {
        ...
        foreach (QWidget *widg, sellersWidgets)
        {
            if (i == 0)
            {
                ...
                {
                    settings.setValue(
                        "account" + QString::number(i + 1), accountsCombo->itemText(i));
                }

                for (int i = 0; i < settings.value("telsCount").toInt(); ++i)
                {
                    settings.setValue("phone" + QString::number(i + 1), telsCombo->itemText(i));
                }

                for (int i = 0; i < settings.value("faxesCount").toInt(); ++i)
                {
                    settings.setValue("fax" + QString::number(i + 1), faxesCombo->itemText(i));
                }

                for (int i = 0; i < settings.value("emailsCount").toInt(); ++i)
                {
                    settings.setValue("email" + QString::number(i + 1), emailsCombo->itemText(i));
                }
            }
            else
            {
                ...
                settings.setValue("district", widg->findChild<QLineEdit *>("districtEdit")->text());
                settings.setValue(
                    "municipality", widg->findChild<QLineEdit *>("municipalityEdit")->text());
                settings.setValue("post", widg->findChild<QLineEdit *>("postEdit")->text());
                settings.setValue("province", widg->findChild<QLineEdit *>("provinceEdit")->text());
                settings.setValue("codeLand", widg->findChild<QLineEdit *>("codeLandEdit")->text());
                ...
                if (widg->findChild<QCheckBox *>()->isChecked())
                {
                    ...
                    settings.setValue(
                        "district", widg->findChild<QLineEdit *>("districtEdit")->text());
                    settings.setValue(
                        "municipality", widg->findChild<QLineEdit *>("municipalityEdit")->text());
                    settings.setValue("post", widg->findChild<QLineEdit *>("postEdit")->text());
                    settings.setValue(
                        "province", widg->findChild<QLineEdit *>("provinceEdit")->text());
                    settings.setValue(
                        "codeLand", widg->findChild<QLineEdit *>("codeLandEdit")->text());
                    settings.setValue(
    ...
    if (!accountEdit->text().isEmpty())
    {
        if (!accountEdit->text().isEmpty() && !bankNameEdit->text().isEmpty()
            ...
            accountsCombo->addItem(
                accountEdit->text().trimmed() + " | " + bankNameEdit->text().trimmed() + " | "
...
void User::on_addTel_clicked()
{
    ...
    if (!phonEdit->text().isEmpty())
    {
        if (telsCombo->findText(phonEdit->text().trimmed()) == -1)
        {
            ...
            QMessageBox::warning(
                this, "Istniejacy numer", "Taki numer znajduje sie juz na Twojej liscie ");
    }
    else
        QMessageBox::warning(
            this,
            "Pusta rubryka dla numeru telefonu",
...
void User::on_addEmail_clicked()
{
    ...
    if (!emailEdit->text().isEmpty())
    {
        if (emailsCombo->findText(emailEdit->text().trimmed()) == -1)
        {
            ...
            QMessageBox::warning(
                this, "Istniejący adres", "Taki adres znajduje się już na Twojej liście ");
    }
    else
        QMessageBox::warning(
            this,
            "Pusta rubryka dla adresu email",
...
void User::prepareFor_0_7_1(bool cameFromOlder)
{
    ...
    if (cameFromOlder)
    {
        ...
        {
            if ((account.isNull() || account.isEmpty()) && (bank.isNull() || bank.isEmpty()))
                togetherAcc = QString();
            else
                togetherAcc += " | " + swift;
        }
