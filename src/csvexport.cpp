...
void CsvExport::prepareConnections()
{
    ...
    connect(ui->browseButton, &QAbstractButton::clicked, this, &CsvExport::choosePathCsv);
    connect(ui->okButton, &QAbstractButton::clicked, this, &CsvExport::createCsvFiles);
...
void CsvExport::choosePathCsv()
{
    ...
    QString directory = QDir::toNativeSeparators(
        QFileDialog::getExistingDirectory(this, tr("Find Files"), sett().getCSVDir()));
...
void CsvExport::splitCsvTasks(bool expBuyers, bool expGoods, bool expInvoices, bool expWarehouses)
{
    ...
    if (expBuyers)
        createBuyersCsvFiles(sett().value("csv_format").toString());
    if (expGoods)
        createProductsCsvFiles(sett().value("csv_format").toString());
    if (expInvoices)
        createInvoicesCsvFiles(
            sett().value("csv_format").toString(),
    ...
    QMessageBox::information(
        ...
        "Eksport danych dobiegł końca. Zaleca się sprawdzenie, czy pliki faktycznie znajdują się "
...
void CsvExport::createBuyersCsvFiles(QString format)
{
    ...
    QString row = "id_klient" + separator(format) + "typ" + separator(format) + "nazwa"
        + separator(format) + "ulica" + separator(format) + "miejscowosc" + separator(format)
        + "kod_pocztowy" + separator(format) + "tel" + separator(format) + "email"
        + separator(format) + "fax" + separator(format) + "NIP" + separator(format) + "nazwa_banku"
        + separator(format) + "nr_konta" + separator(format) + "bic_swift" + separator(format)
        + "krs" + separator(format) + "strona_www";
    ...
    for (int i = 0; i < buyersXml.size(); ++i)
    {
        ...
        idCvsBuyer.insert(
            buyTic.remove(QChar('-')),
            QString::number(i + 1)); // according to the tic, because the number is always unique
        ...
        QString next_row = QString::number(i + 1) + separator(format)
            + preventNull(buyersXml.at(i).type) + separator(format)
            + preventNull(buyersXml.at(i).name) + separator(format)
            + preventNull(buyersXml.at(i).address) + separator(format)
            + preventNull(buyersXml.at(i).place) + separator(format)
            + preventNull(buyersXml.at(i).code) + separator(format)
            + preventNull(buyersXml.at(i).phone) + separator(format)
            + preventNull(buyersXml.at(i).email) + separator(format)
            + preventNull(buyersXml.at(i).fax) + separator(format)
            + preventNull(buyersXml.at(i).tic) + separator(format)
            + preventNull(buyersXml.at(i).bank) + separator(format)
            + preventNull(buyersXml.at(i).account) + separator(format)
            + preventNull(buyersXml.at(i).swift) + separator(format)
            + preventNull(buyersXml.at(i).krs) + separator(format)
            + preventNull(buyersXml.at(i).www);
...
void CsvExport::createProductsCsvFiles(QString format)
{
    ...
    QString row = "id_produkt" + separator(format) + "typ" + separator(format) + "nazwa"
        + separator(format) + "vat" + separator(format) + "opis" + separator(format) + "kod"
        + separator(format) + "pkwiu" + separator(format) + "jednostka" + separator(format)
        + "netto1" + separator(format) + "netto2" + separator(format) + "netto3" + separator(format)
        + "netto4";
    ...
    for (int i = 0; i < productsXml.size(); ++i)
    {
        ...
        else
        {
            idCvsProducts.insert(
                ...
                QString::number(i + 1)); // sometimes there is no code given for some product
            qDebug() << "productsXml.at(i).name: " << productsXml.at(i).name;
            qDebug() << "idCvsProducts.count(): " << idCvsProducts.count();
        }

        QString next_row = QString::number(i + 1) + separator(format)
            + preventNull(productsXml.at(i).type) + separator(format)
            + preventNull(productsXml.at(i).name) + separator(format)
            + preventNull(QString::number(productsXml.at(i).vat)) + separator(format)
            + preventNull(productsXml.at(i).desc) + separator(format)
            + preventNull(productsXml.at(i).code) + separator(format)
            + preventNull(productsXml.at(i).pkwiu) + separator(format)
            + preventNull(productsXml.at(i).quanType) + separator(format)
            + preventNull(sett().numberToString(productsXml.at(i).prices[0], 'f', 2))
            + separator(format)
            + preventNull(sett().numberToString(productsXml.at(i).prices[1], 'f', 2))
            + separator(format)
            + preventNull(sett().numberToString(productsXml.at(i).prices[2], 'f', 2))
            + separator(format)
            + preventNull(sett().numberToString(productsXml.at(i).prices[3], 'f', 2));
...
void CsvExport::createSellersCsvFiles(QString format)
{
    ...
    QString row = "id_sprzedawca" + separator(format) + "nazwa" + separator(format) + "adres"
        + separator(format) + "miejscowosc" + separator(format) + "kod_pocztowy" + separator(format)
        + "oddzial" + separator(format) + "NIP" + separator(format) + "REGON" + separator(format)
        + "KRS" + separator(format) + "nazwa_banku" + separator(format) + "nr_konta"
        + separator(format) + "SWIFT_BIC" + separator(format) + "tel" + separator(format) + "fax"
        + separator(format) + "email" + separator(format) + "www";

    in << row << endl;

    idCvsSeller.insert(
        preventNull(settings.value("address").toString()),
        QString::number(1)); // address is the best choice, if there are other branches

    QString next_row = QString::number(1) + separator(format)
        + preventNull(settings.value("name").toString()) + separator(format)
        + preventNull(settings.value("address").toString()) + separator(format)
        + preventNull(settings.value("city").toString()) + separator(format)
        + preventNull(settings.value("zip").toString()) + separator(format) + "glowny"
        + separator(format) + preventNull(settings.value("tic").toString()) + separator(format)
        + preventNull(settings.value("regon").toString()) + separator(format)
        + preventNull(settings.value("krs").toString()) + separator(format)
        + preventNull(settings.value("bank").toString()) + separator(format)
        + preventNull(settings.value("account").toString()) + separator(format)
        + preventNull(settings.value("swift").toString()) + separator(format)
        + preventNull(settings.value("phone").toString()) + separator(format)
        + preventNull(settings.value("fax").toString()) + separator(format)
        + preventNull(settings.value("email").toString()) + separator(format)
        + preventNull(settings.value("website").toString());
    ...
    for (int i = 1; i < settings.value("sellerCount").toInt(); ++i)
    {
        ...
        idCvsSeller.insert(
            preventNull(setts.value("address").toString()),
            QString::number(i + 1)); // address is the best
                                     // choice, if there are
                                     // other branches
                                     // (unique value is
                                     // needed)
        ...
        QString next_row = QString::number(i + 1) + separator(format)
            + preventNull(setts.value("name").toString()) + separator(format)
            + preventNull(setts.value("address").toString()) + separator(format)
            + preventNull(setts.value("city").toString()) + separator(format)
            + preventNull(setts.value("zip").toString()) + separator(format) + "poboczny"
            + separator(format) + preventNull(setts.value("tic").toString()) + separator(format)
            + preventNull(setts.value("regon").toString()) + separator(format)
            + preventNull(setts.value("krs").toString()) + separator(format)
            + preventNull(setts.value("bank").toString()) + separator(format)
            + preventNull(setts.value("account").toString()) + separator(format)
            + preventNull(setts.value("swift").toString()) + separator(format)
            + preventNull(setts.value("phone").toString()) + separator(format)
            + preventNull(setts.value("fax").toString()) + separator(format)
            + preventNull(setts.value("email").toString()) + separator(format)
            + preventNull(setts.value("website").toString());
...
void CsvExport::createInvoicesCsvFiles(QString format, QDate from, QDate to)
{
    ...
    QString row = "id_faktura" + separator(format) + "typ,data_sprzedazy" + separator(format)
        + "data_duplikatu" + separator(format) + "data_wystawienia" + separator(format)
        + "data_zaplaty" + separator(format) + "id_klient" + separator(format) + "id_sprzedawca"
        + separator(format) + "id_produkt1" + separator(format) + "id_produkt2" + separator(format)
        + "id_produkt3" + separator(format) + "id_produkt4" + separator(format) + "ilosc1"
        + separator(format) + "netto1" + separator(format) + "brutto1" + separator(format)
        + "ilosc2" + separator(format) + "netto2" + separator(format) + "brutto2"
        + separator(format) + "ilosc3" + separator(format) + "netto3" + separator(format)
        + "brutto3" + separator(format) + "ilosc4" + separator(format) + "netto4"
        + separator(format) + "brutto4" + separator(format) + "sposob_platnosci" + separator(format)
        + "waluta";
    ...
    for (int i = 0; i < invoicesXml.size(); ++i)
    {
        InvoiceData invData = dl->invoiceSelectData(
            invoicesXml.at(i).id, getInvoiceTypeFullName(invoicesXml.at(i).type), true);
        ...
        QString id_customer =
            (invData.custTic.remove('-') != "") ? idCvsBuyer[invData.custTic.remove('-')] : "null";
        // invData.custTic.remove('-'), because of validation choice option. When
        // validation on data is unchecked, users can put tic into app without '-'
        // characters
        ...
        QString id_seller =
            (invData.sellerAddress != "") ? idCvsSeller[invData.sellerAddress] : "null";
        ...
        for (int k = 0; k < invData.products.count(); k++)
        {
            ...
            if (k == invData.products.count() - 1)
            { // if we are at the end of loop
                if (k < 3)
                { // if number of last transaction was less than 3
                    for (int j = 4 - (3 - k); j < 4; j++)
                    { // we want fill values with "null", only if there aren't 4
        ...
        QString paymentMethod = (!invData.paymentType.isEmpty() && !invData.paymentType.isNull())
            ? invData.paymentType
            : "null";
        QString currency = (!invData.currencyType.isEmpty() && !invData.currencyType.isNull())
        ...
        QString next_row = QString::number(i + 1) + separator(format) + type + separator(format)
            + sellDate + separator(format) + duplDate + separator(format) + prodDate
            + separator(format) + payDate + separator(format) + id_customer + separator(format)
            + id_seller + separator(format) + id_products.at(0) + separator(format)
            + id_products.at(1) + separator(format) + id_products.at(2) + separator(format)
            + id_products.at(3) + separator(format) + quantity1 + separator(format) + nett1
            + separator(format) + gross1 + separator(format) + quantity2 + separator(format) + nett2
            + separator(format) + gross2 + separator(format) + quantity3 + separator(format) + nett3
            + separator(format) + gross3 + separator(format) + quantity4 + separator(format) + nett4
            + separator(format) + gross4 + separator(format) + paymentMethod + separator(format)
            + currency;
    ...
    for (int i = 0; i < wareXml.size(); ++i)
    {
        ...
        for (int k = 0; k < invData.products.count(); k++)
        {
            ...
            if (k == 0)
            {
                quantity1 = sett().numberToString(invData.products[k].quantity, 'f', 2);
                nett1 = sett().numberToString(invData.products[k].nett, 'f', 2);
                givAm1 = sett().numberToString(invData.products[k].givedOutAmount, 'f', 2);
                reqAm1 = sett().numberToString(invData.products[k].requiredAmount, 'f', 2);
            }
            if (k == 1)
            {
                quantity2 = sett().numberToString(invData.products[k].quantity, 'f', 2);
                nett2 = sett().numberToString(invData.products[k].nett, 'f', 2);
                givAm2 = sett().numberToString(invData.products[k].givedOutAmount, 'f', 2);
                reqAm2 = sett().numberToString(invData.products[k].requiredAmount, 'f', 2);
            }
            if (k == 2)
            {
                quantity3 = sett().numberToString(invData.products[k].quantity, 'f', 2);
                nett3 = sett().numberToString(invData.products[k].nett, 'f', 2);
                givAm3 = sett().numberToString(invData.products[k].givedOutAmount, 'f', 2);
                reqAm3 = sett().numberToString(invData.products[k].requiredAmount, 'f', 2);
            }
            if (k == 3)
            {
                quantity4 = sett().numberToString(invData.products[k].quantity, 'f', 2);
                nett4 = sett().numberToString(invData.products[k].nett, 'f', 2);
                givAm4 = sett().numberToString(invData.products[k].givedOutAmount, 'f', 2);
                reqAm4 = sett().numberToString(invData.products[k].requiredAmount, 'f', 2);
            }
        ...
        if (quantity1.isEmpty() && nett1.isEmpty() && givAm1.isEmpty() && reqAm1.isEmpty())
        {
            quantity1 = "null";
            nett1 = "null";
            givAm1 = "null";
        ...
        if (quantity2.isEmpty() && nett2.isEmpty() && givAm2.isEmpty() && reqAm2.isEmpty())
        {
            quantity2 = "null";
            nett2 = "null";
            givAm2 = "null";
        ...
        if (quantity3.isEmpty() && nett3.isEmpty() && givAm3.isEmpty() && reqAm3.isEmpty())
        {
            quantity3 = "null";
            nett3 = "null";
            givAm3 = "null";
        ...
        if (quantity4.isEmpty() && nett4.isEmpty() && givAm4.isEmpty() && reqAm4.isEmpty())
        {
            quantity4 = "null";
            nett4 = "null";
            givAm4 = "null";
        ...
        QString next_row = QString::number(i + 1) + separator(format) + type + separator(format)
            + sellDate + separator(format) + duplDate + separator(format) + prodDate
            + separator(format) + giveDate + separator(format) + takeDate + separator(format)
            + givePlace + separator(format) + takePlace + separator(format) + departCost
            + separator(format) + id_customer + separator(format) + id_seller + separator(format)
            + id_products.at(0) + separator(format) + id_products.at(1) + separator(format)
            + id_products.at(2) + separator(format) + id_products.at(3) + separator(format)
            + quantity1 + separator(format) + nett1 + separator(format) + givAm1 + separator(format)
            + reqAm1 + separator(format) + quantity2 + separator(format) + nett2 + separator(format)
            + givAm2 + separator(format) + reqAm2 + separator(format) + quantity3
            + separator(format) + nett3 + separator(format) + givAm3 + separator(format) + reqAm3
            + separator(format) + quantity4 + separator(format) + nett4 + separator(format) + givAm4
            + separator(format) + reqAm4 + separator(format) + paymentMethod;
