    ...
    , allData(data)
{
    ...
    saveXmlFile();
    findErrors(
        allData.value("jpkFileArt"),
        getContentFromFileJPK(
            sett().getJPKDir() + "/" + QDate::currentDate().toString("yyyyMMdd")
            + allData.value("jpkFileArt")));
}
...
void SaftfileOutput::saveXmlFile()
{
    ...
    // prepareSchema(allData.value("jpkFileArt")).toString());
    ...
    xmlWriter.writeTextElement(
        "tns:WariantFormularza", prepareFileVarriant(allData.value("jpkFileArtWithVersion")));
    xmlWriter.writeTextElement(
        "tns:CelZlozenia",
        prepareAppPurposeNr(allData.value("jpkFileArt"), allData.value("applicationPurpose")));
    xmlWriter.writeTextElement(
        "tns:DataWytworzeniaJPK", QDateTime::currentDateTime().toString("yyyy-MM-ddTHH:mm:ss"));
...
QString SaftfileOutput::prepareTns(QString fileArt)
{
    if (fileArt == "JPK_VAT")
        ...
        return QString("http://jpk.mf.gov.pl/wzor/2016/03/09/03095/");
...
{
    if (fileArt == "JPK_FA" && filePurpose == "Standardowe")
        return "1";
    else if (fileArt == "JPK_FA" && filePurpose == "Korekta")
        return allData.value("correctionNr");
    else if (fileArt == "JPK_VAT" && filePurpose == "Standardowe")
        return "0";
    else if (fileArt == "JPK_VAT" && filePurpose == "Korekta")
        return allData.value("correctionNr"); // in range 1 - 9
...
void SaftfileOutput::saveXmlFileJKP_VAT(QXmlStreamWriter &xmlWriter)
{
    ...
    for (int i = 0; i < invoices.size(); ++i)
    {
        ...
        xmlWriter.writeTextElement(
            "tns:DataWystawienia", invoices.at(i).productDate.toString("yyyy-MM-dd"));
        xmlWriter.writeTextElement(
            "tns:DataSprzedazy", invoices.at(i).sellingDate.toString("yyyy-MM-dd"));
    ...
    xmlWriter.writeTextElement(
        "tns:PodatekNalezny", sett().numberToString(vatPriceSum).remove(','));
    xmlWriter.writeEndElement();
...
void SaftfileOutput::saveXmlFileJKP_FA(QXmlStreamWriter &xmlWriter)
{
    ...
    double gross = 0;
    double sumBasicNet1 =
        0.00; // now is 23% or 22% VAT as basic; sum of basic net sums, which have 23 %
    ...
    for (int i = 0; i < invoices.size(); ++i)
    {
        ...
        if (invTypeJPK == "KOREKTA")
        {
            ...
            xmlWriter.writeTextElement(
                "tns:OkresFaKorygowanej", invoices.at(i).productDate.toString("yyyy-MM-dd"));
        }
        else if (invTypeJPK == "ZAL")
        {
            double givenSum = 0.00;
            double vatSum = static_cast<float>(invoices.at(i).products.value(0).vat);
            if (invoices.at(i).custPaym.date1 < QDate::currentDate())
                givenSum += invoices.at(i).custPaym.amount1;
            if (invoices.at(i).custPaym.date2 < QDate::currentDate())
                givenSum += invoices.at(i).custPaym.amount2;
            xmlWriter.writeTextElement(
                "tns:ZALZaplata", QString::number(givenSum, 'f', 2).remove(','));
            xmlWriter.writeTextElement(
                "tns:ZALPodatek",
                QString::number((givenSum * vatSum) / (100 + vatSum), 'f', 2).remove(','));
        }
    ...
    if (sortedVats.contains("0.08"))
        xmlWriter.writeTextElement(QString("tns:Stawka2"), QString("0.08"));
    else if (sortedVats.contains("0.07"))
        xmlWriter.writeTextElement(QString("tns:Stawka2"), QString("0.07"));
    else
        xmlWriter.writeTextElement(QString("tns:Stawka2"), QString("0.08"));
    ...
    if (sortedVats.contains("0.07") && sortedVats.contains("0.08"))
        xmlWriter.writeTextElement(QString("tns:Stawka4"), QString("0.07"));
    else if (sortedVats.contains("0.04"))
        xmlWriter.writeTextElement(QString("tns:Stawka4"), QString("0.04"));
    else
        xmlWriter.writeTextElement(QString("tns:Stawka4"), QString("0.00"));

    if ((sortedVats.contains("0.07") && sortedVats.contains("0.08")) && sortedVats.contains("0.04"))
        xmlWriter.writeTextElement(QString("tns:Stawka5"), QString("0.04"));
    else
        xmlWriter.writeTextElement(QString("tns:Stawka5"), QString("0.00"));
    xmlWriter.writeEndElement();
    ...
    for (int i = 0; i < invoices.size(); ++i)
    {
        ...
        for (int j = 0; j < products1.count(); j++)
        {
            ...
            xmlWriter.writeTextElement(
                "tns:P_9A",
                QString("%1").arg(products1.at(j).nett * (products1.at(j).discount / 100)));
            xmlWriter.writeTextElement(
                "tns:P_9B",
                QString("%1").arg(
                    products1.at(j).price
                    + (products1.at(j).price * (static_cast<float>(products1.at(j).vat) / 100))));
...
const QString
SaftfileOutput::getInvTypeForJPKFA(QString abbrInv, QString paymentType, QChar beginFilename)
{
    qDebug() << "Abbreviation of invoice is: " << beginFilename;
    if ((abbrInv == "korekta") || (beginFilename == 'k'))
        return "KOREKTA";
    else if (paymentType == "zaliczka")
        return "ZAL";
    else if ((abbrInv == "FVAT") || (abbrInv == "FBrutto"))
        return "VAT";
    else
        return "POZ";
...
void SaftfileOutput::findErrors(QString fileArt, const QString content)
{
    ...
    // %3").arg(messageHandler.line()).arg(messageHandler.column()).arg(statusMessage));
    msgBox.setText(
        QString("Dopóki błędy związane ze stosowaniem validatora plików xsd nie zostaną "
                "ostatecznie naprawione w bibliotece Qt5, walidacja w programie nie będzie ujęta. "
                "Zaleca się użycie walidatora na jednej ze stron internetowych, jak: \n "
                "https://www.freeformatter.com/xml-validator-xsd.html \n Zaleca się także "
                "dodatkowe sprawdzenie dokumentów z pomocą osób specjalizujących się w księgowości "
                "przed wysłaniem dokumentu"));
    // msgBox.setInformativeText("Co chcesz zrobić? Anulować? Zapisać plik z treścią błędów do
    // katalogu plików JPK? Poprawić treść?");
    msgBox.setInformativeText(
        "Co chcesz zrobić? Anulować? Skopiować treść schematu xsd do schowka? Skopiować treść "
        "pliku JPK do schowka? Poprawić treść? Wybór kopiowania nie zamyka okna.");
...
void SaftfileOutput::saveErrors(MessageHandler *messageHandler)
{
    ...
    outfile.setFileName(
        sett().getJPKDir() + "/" + QDate::currentDate().toString("yyyyMMdd")
        + allData.value("jpkFileArt") + "-Errors");
...
void SaftfileOutput::createNoteJPKWindow(MessageHandler *messageHandler)
{
    ...
    if (guard.tryToRun())
    {
        ...
        inputTextFile(getContentFromFileJPK(
            sett().getJPKDir() + "/" + QDate::currentDate().toString("yyyyMMdd")
            + allData.value("jpkFileArt")));
        if (messageHandler != 0)
            statusText.data()->setText(messageHandler->statusMessage());
        if (messageHandler != 0)
...
void SaftfileOutput::overwriteFile()
{
    ...
    if (out.atEnd())
        QMessageBox::information(this, "Nadpisywanie", "Operacja zakończona sukcesem");
