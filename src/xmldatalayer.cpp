...
// ************ KONTRAHENCI START *****************
// helper method
void XmlDataLayer::buyersElemToData(BuyerData &o_buyerData, QDomElement const &i_element)
{
    StrDebug();
...
void XmlDataLayer::invoiceSellerDataToElem(DocumentData &, QDomElement &o_element)
{
    StrDebug();
...
void XmlDataLayer::invoiceBuyerDataToElem(DocumentData &i_invData, QDomElement &o_element)
{
    StrDebug(<< i_invData.getCustomer());
...
void XmlDataLayer::invoiceProdDataToElem(
    ...
    int currentRow)
{
    StrDebug(<< i_prodData.id << i_prodData.name << i_prodData.quantity << i_prodData.quanType);
...
void XmlDataLayer::warehouseProdDataToElem(
    const ProductData &i_prodData,
    QDomElement &o_element,
    int type)
{
...
InvoiceData XmlDataLayer::invoiceSelectData(QString name, int type, bool onlyCheck)
{
    ...
    if (onlyCheck && (domName == sett().getCorrDocName()))
        o_invData.origInvNr = root.attribute("originalInvoiceNo");
    o_invData.id = name;
    ...
    if (onlyCheck && (domName == sett().getCorrDocName()))
        o_invData.reason = additional.attribute("reason");
...
WarehouseData XmlDataLayer::warehouseSelectData(QString name, int type, bool onlyCheck)
{
    StrDebug();
    ...
    if (type == 10)
    {
        ...
        o_invData.goodFromDate =
            QDate::fromString(root.attribute("goodFromDate"), sett().getDateFormat());
...
QVector<InvoiceData> XmlDataLayer::invoiceSelectAllData(QDate start, QDate end, bool onlyCheck)
{
    StrDebug();
    ...
    // finds invoices between years with difference more than 1. It's better to look in directories
    // between searched years than in all in main directory
    ...
    for (i = 0; i < max; ++i)
    {
        ...
        if (nameFilter(
                files[i], start, end, sett().getInoiveDocName(), QFileInfo(file).absolutePath()))
        {
            if (!onlyCheck)
            {
                invDt.id = files[i];
...
QVector<WarehouseData> XmlDataLayer::warehouseSelectAllData(QDate start, QDate end)
{
    StrDebug();
    ...
    if (QDate::currentDate().year() != end.year())
        searchAllDirs.append(sett().getWarehouseFullDir() + QString::number(end.year()));
    else
    ...
    for (i = 0; i < max; ++i)
    {
        ...
        if (nameFilter(
                files[i], start, end, sett().getWarehouseDocName(), QFileInfo(file).absolutePath()))
        {
            invDt.id = files[i];
            root = doc.documentElement();
            invDt.invNr = root.attribute("no");
...
bool XmlDataLayer::invoiceInsertData(InvoiceData &oi_invData, int type)
{
    ...
    root.setAttribute("endTransDate", oi_invData.endTransDate.toString(sett().getDateFormat()));
    if (type == 8)
    {
...
bool XmlDataLayer::warehouseInsertData(WarehouseData &oi_invData, int type)
{
    ...
    if (fileName == "")
    {
        ...
        int pNumber = 0;
        file.setFileName(
            sett().getWarehouseFullDir() + "m" + fileName + "_" + sett().numberToString(pNumber)
            + ".xml");
        oi_invData.id = "m" + fileName + "_" + sett().numberToString(pNumber) + ".xml";
        pNumber += 1;
