...
{
    qDebug() << QString("QJsonObject[dane] of value ") << listJsonNodes.at(0) << QString(": ")
             << item[listJsonNodes.at(0)];
    ...
    qDebug() << QString("QJsonObject[dane] of value ") << listJsonNodes.at(1) << QString(": ")
             << item[listJsonNodes.at(1)];
    QJsonValue city = item[listJsonNodes.at(1)];
    placeEdit->setText(city.toString().trimmed());

    qDebug() << QString("QJsonObject[dane] of value ") << listJsonNodes.at(2) << QString(" and ")
             << listJsonNodes.at(3) << QString(": ") << item[listJsonNodes.at(2)];
    ...
    qDebug() << QString("QJsonObject[dane] of value ") << listJsonNodes.at(4) << QString(": ")
             << item[listJsonNodes.at(4)];
    QJsonValue postalCode = item[listJsonNodes.at(4)];
    codeEdit->setText(postalCode.toString().insert(2, '-').trimmed());

    qDebug() << QString("QJsonObject[dane] of value ") << listJsonNodes.at(5) << QString(": ")
             << item[listJsonNodes.at(5)];
    QJsonValue tel = item[listJsonNodes.at(5)];
    telefonEdit->setText("+48" + tel.toString().trimmed());

    qDebug() << QString("QJsonObject[dane] of value ") << listJsonNodes.at(6) << QString(": ")
             << item[listJsonNodes.at(6)];
    QJsonValue email = item[listJsonNodes.at(6)];
    emailEdit->setText(email.toString().toLower().trimmed());

    qDebug() << QString("QJsonObject[dane] of value ") << listJsonNodes.at(7) << QString(": ")
             << item[listJsonNodes.at(7)];
...
void Buyers::createComposerJson(QString path, QString filename)
{
    ...
    recordObject.insert("description", QJsonValue::fromVariant("Gus Api Library for PHP"));
...
void Buyers::on_gusBtn_clicked()
{
    ...
    if (nipEdit->text().isEmpty() || nipEdit->text().isNull())
        QMessageBox::warning(
            this, "NIP", "Aby skorzystać z funkcji, powinieneś najpierw podać numer NIP");
    else
    {
        ...
        if (!QFile(QDir::homePath() + "/.local/share/data/elinux/gus/composer.json").exists())
        {
