    ...
    : Warehouse(parent, dl, in_form)
{
    ...
    connect(
        lineEdit2, SIGNAL(textEdited(const QString &)), this, SLOT(emitChange(const QString &)));
    connect(
        lineEdit3, SIGNAL(textEdited(const QString &)), this, SLOT(emitChange(const QString &)));
    connect(
        dateTimeEdit1, SIGNAL(dateChanged(const QDate &)), this, SLOT(emitChange(const QDate &)));
    connect(
        dateTimeEdit2, SIGNAL(dateChanged(const QDate &)), this, SLOT(emitChange(const QDate &)));
...
GoodsIssuedNotes::~GoodsIssuedNotes()
{
    ...
    disconnect(
        lineEdit1, SIGNAL(textEdited(const QString &)), this, SLOT(emitChange(const QString &)));
    disconnect(
        lineEdit2, SIGNAL(textEdited(const QString &)), this, SLOT(emitChange(const QString &)));
    disconnect(
        lineEdit3, SIGNAL(textEdited(const QString &)), this, SLOT(emitChange(const QString &)));
...
void GoodsIssuedNotes::readWarehouseData(QString invFile)
{
    ...
    dateTimeEdit1->setDate(
        QDate::fromString(root.attribute("goodFromDate"), sett().getDateFormat()));
    dateTimeEdit2->setDate(QDate::fromString(root.attribute("goodToDate"), sett().getDateFormat()));
    ...
    for (i = 0; i < goodsCount; ++i)
    {
        ...
        tableGoods->setItem(i, 7, new QTableWidgetItem(good.attribute(goodsColumns[4])));
        tableGoods->setItem(i, 8, new QTableWidgetItem(good.attribute(goodsColumns[5])));
        tableGoods->setItem(i, 9, new QTableWidgetItem(good.attribute(goodsColumns[6])));
        tableGoods->setItem(i, 10, new QTableWidgetItem(good.attribute(goodsColumns[7])));
...
void GoodsIssuedNotes::makeInvoiceSumm()
{
    ...
    invStrList += "<td width=\"21%\" align=\"center\">" + trUtf8("Skąd") + "</td>"; // net
    invStrList += "<td width=\"21%\" align=\"center\">" + trUtf8("Dokąd") + "</td>"; // vat
    invStrList +=
        "<td width=\"31%\" align=\"center\">" + trUtf8("Wydział/Dział") + "</td>"; // brutto
    ...
    invStrList +=
        "<tr><table align=\"right\" width=\"100%\" border=\"0\" "
        "cellspacing=\"0\" cellpadding=\"5\">";
    invStrList += "<tr class=\"productsSumHeader\" valign=\"middle\" width=\"100%\">";
    invStrList += "<td id=\"notNec\" width=\"10%\" align=\"center\">&nbsp;</td>"; // TUTAJ
    invStrList += "<td width=\"11%\" align=\"center\">" + trUtf8("Razem netto: ") + "</td>"; // net
...
void GoodsIssuedNotes::editGoods()
{
    ...
    if (cur >= 0)
    {
        ...
        if (changeQuant->exec() == QDialog::Accepted)
        {
            ...
            tableGoods->item(currentRow, 9)->setText(changeQuant->requiredAmBox->cleanText());
            tableGoods->item(currentRow, 10)->setText(changeQuant->givedOutBox->cleanText());

            double changedNet = sett().stringToDouble(tableGoods->item(currentRow, 7)->text())
