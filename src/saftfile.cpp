...
QVector<InvoiceData> Saftfile::getInvFromRange()
{
    ...
    if (ui->jpksFromDate->date().year() < 2018)
    {
        QMessageBox::information(
            this,
            "Błąd przedziału czasu",
            "Data początkowa nie może sięgać daty wcześniejszej niż 1 stycznia 2018 roku. Jest to "
            "spowodowane tym, że program używa generatora dla JPK_VAT w wersji 3, ważnej od 2018 "
            "roku.");
...
void Saftfile::putIntoTable(QVector<InvoiceData> invoices)
{
    ...
    {
        QMessageBox::information(
            this, "brak faktur", "Brakuje jakiejkolwiek faktury z tego okresu");
        return;
    }
    ...
    for (int i = 0; i < invoices.size(); ++i)
    {
        ...
        ui->saftInvoicesTable->item(ui->saftInvoicesTable->rowCount() - 1, 0)
            ->setText(QString::number(i + 1));
        ui->saftInvoicesTable->item(ui->saftInvoicesTable->rowCount() - 1, 1)
            ->setText(invoices.at(i).invNr);
        ui->saftInvoicesTable->item(ui->saftInvoicesTable->rowCount() - 1, 2)
            ->setText(QString::number(gross));
        ui->saftInvoicesTable->item(ui->saftInvoicesTable->rowCount() - 1, 3)
            ->setText(invoices.at(i).sellingDate.toString(sett().getDateFormat()));
        ...
        ui->saftInvoicesTable->item(ui->saftInvoicesTable->rowCount() - 1, 3)
            ->setTextAlignment(Qt::AlignCenter);
        ui->saftInvoicesTable->item(ui->saftInvoicesTable->rowCount() - 1, 4)
            ->setTextAlignment(Qt::AlignCenter);
    }
...
QVector<InvoiceData> Saftfile::addSAFTFieldsToList(QVector<InvoiceData> invoices)
{
    ...
    for (int i = 0; i < invoices.size(); ++i)
    {
        ...
        {
            // QString textSAFT =
            // qobject_cast<QComboBox*>(ui->saftInvoicesTable->cellWidget(i,4))->currentText();
    ...
    if ((getJpkFileArt() == "JPK_VAT") || (getJpkFileArt() == "JPK_FA"))
    {
        ...
        while (invoice != inv.constEnd())
        {
            ...
            if (invoice->type == "korekta")
            {
                ...
                {
                    qDebug() << "(" << invNr->origInvNr << "==" << invoice->invNr << ") && ("
                             << invNr->type << " == FVAT )";
                    if ((invNr->invNr == invoice->origInvNr)
                        && ((invNr->type == "FVAT") ^ (invNr->type == "FBrutto")))
                        fvatFound = true;
...
void Saftfile::initInvoicesRange()
{
    ...
    if (ui->saftInvoicesTable->rowCount() > 0)
        ui->generationBtn->setDisabled(false);
    else
        ui->generationBtn->setDisabled(true);
}
...
void Saftfile::prepareNextStep()
{
    ...
    if ((getJpkFileArt() == "JPK_FA") && getTaxOfficeNr().isEmpty())
    {
        QMessageBox::information(
            this,
            "Brak danych",
            "W przypadku próby wygenerowania pliku JPK_FA, trzeba wprowadzić numer urzędu "
            "skarbowego, do którego plik będzie wysyłany. ");
    }
    else
    {
        ...
        if (guard.tryToRun())
        {
            QPointer<SaftfileOutput> saftfileoutput =
                new SaftfileOutput(addSAFTFieldsToList(invs), data);
