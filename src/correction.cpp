...
bool Correction::saveInvoice()
{
    ...
    root.setAttribute("endTransDate", endTransactionDate->date().toString(sett().getDateFormat()));
...
void Correction::makeBeforeCorrProducts()
{
         ...
         ++iter)
    {
        ...
        if (sett().value("amount").toBool())
            invStrList +=
                "<td>" + sett().numberToString(iter.value().getQuantity(), 'f', 0) + "</td>";
        ...
        if (sett().value("discountperc").toBool())
            invStrList += "<td>" + sett().numberToString(iter.value().getDiscount(), 'f', 0)
                + "%</td>"; // discount
...
void Correction::makeInvoiceSummAll()
{
    ...
    invStrList += trUtf8("Słownie: ") + sumCorrected + "<br/>";

    delete conv;

    if (paysCombo->currentText() == trUtf8("gotówka"))
    {
        invStrList += trUtf8("Sposób płatności: ") + paysCombo->currentText() + "<br/>";
    }
