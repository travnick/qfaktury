...
void Setting::init()
{
    ...
    QList<QCheckBox *> settBoxes = QList<QCheckBox *>()
        << cbMonth << cbYear << shortYear << cbEdit << cbSmbEdit << cbValOn << reqSignIssue
        << reqSignReceipt << userinfoswift << userinfobank << userinfowww << userinfoadress
        << userinfocity << userinfonip << userinfomail << buyerinfowww << buyerinfocity
        << buyerinfoaddress << buyerinfoaccount << buyerinfomail << buyerinfotel << buyerinfotic
        << buyerinfoname << userinfotel << userinfoaccount << userinfoname << userinfofax
        << userinfokrs << cb1 << cb2 << cb3 << cb4 << cb5 << cb6 << cb7 << cb8 << cb9 << cb10
        << cb11 << cb12 << cb13 << cb14 << cbDay;
    ...
    connect(signEdit, SIGNAL(textChanged(const QString &)), this, SLOT(saveBtnEnable()));
    ...
    connect(csvLinePath, SIGNAL(textChanged(const QString &)), this, SLOT(saveBtnEnable()));
    connect(backupLinePath, SIGNAL(textChanged(const QString &)), this, SLOT(saveBtnEnable()));
    connect(CSVUSAFormat, SIGNAL(toggled(bool)), this, SLOT(checkCurrCsvFormat(bool)));
    connect(CSVEURFormat, SIGNAL(toggled(bool)), this, SLOT(checkCurrCsvFormat(bool)));
    connect(noAskCSVPath, SIGNAL(stateChanged(int)), this, SLOT(blockCsvCheckboxPath(int)));
    connect(noAskBackupPath, SIGNAL(stateChanged(int)), this, SLOT(blockBackupCheckboxPath(int)));
    connect(regularBackupBtn, SIGNAL(stateChanged(int)), this, SLOT(blockBackupCheckbox(int)));
...
void Setting::addSignBtnClick()
{
    ...
    QString ofn = QFileDialog::getOpenFileName(
        this,
        trUtf8("Wybierz plik do wstawienia jako podpis"),
        "",
        trUtf8("Obrazki (*.jpg *.png)"));
...
void Setting::checkOnlyOneInterval(bool checked)
{
    if (checked)
    {
        ...
        radios << backupOnceDay << backupOnceWeek << backupOnceMonth << backupEveryHour
               << backupEveryMin;
        ...
        else
        {
            ...
            for (i = radios.begin(); i != radios.end(); ++i)
            {
                if (helpFindRadio[sett().value("backup_interval").toString().back().toLatin1()]
...
void Setting::readSettings()
{
    ...
    checkOnlyOneInterval(sett().value("backup_interval").toString().back() == 'h');
    checkOnlyOneInterval(sett().value("backup_interval").toString().back() == 'm');
    sett().endGroup();
