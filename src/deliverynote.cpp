...
void DeliveryNote::makeInvoiceSummAll()
{
    ...
    invStrList += "<span class=\"toPay\">";
    invStrList +=
        trUtf8("Termin płatności: ") + liabDate->date().toString(sett().getDateFormat()) + "<br/>";
    invStrList += "</span>";
...
bool DeliveryNote::saveInvoice()
{
    ...
    if (this->windowTitle() != s_WIN_WZ_EDIT)
    {
        QMessageBox msgBox;
        msgBox.setText(
            trUtf8("Czy chcesz od razu do tworzonego dokumentu WZ wygenerować fakturę?"));
        msgBox.setInformativeText(
            trUtf8("Jeśli chcesz dodać dodatkowo fakturę do "
