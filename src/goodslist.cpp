...
GoodsList::~GoodsList()
{
    ...
    if (ifGoodIssueNote)
    {
        disconnect(amountOut, SIGNAL(valueChanged(const QString &)), this, SLOT(calcNet()));
...
void GoodsList::init()
{
    ...
    if (ifGoodIssueNote)
        connect(amountOut, SIGNAL(valueChanged(const QString &)), this, SLOT(calcNet()));
    else
    ...
    if (ifGoodIssueNote)
    {
            ...
            && (amountOut->text() == "" || amountOut->value() < 0.001))
        {
            QMessageBox::information(
                this, "QFaktury", trUtf8("Podaj ilość żądaną i ilość wydaną"), QMessageBox::Ok);
...
void GoodsList::calcNet()
{
    ...
    if (listWidget->selectedItems().size() == 1)
    {
        if (ifGoodIssueNote)
        {
            double price = (amountOut->value() * priceBoxEdit->value()); // price * quantity
